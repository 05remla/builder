...
#EXT_ISO_CONTENTS="/root/IMAGE_BUILD_DIRECTORY/ext_contents"
#SYS_BUILD_HOME="/root/IMAGE_BUILD_DIRECTORY"
...
cp ${item1} "${EXT_ISO_CONTENTS}/image/initrd.img"
cp ${item2} "${EXT_ISO_CONTENTS}/image/vmlinuz"

touch "${EXT_ISO_CONTENTS}/image/DEBIAN_CUSTOM"

grub-mkstandalone \
  --format=i386-pc \
  --output=${EXT_ISO_CONTENTS}/scratch/core.img \
...
cat /usr/lib/grub/i386-pc/cdboot.img \
    ${EXT_ISO_CONTENTS}/scratch/core.img \
...
xorriso \
  ...
  -output "${SYS_BUILD_HOME}/debian-custom.iso" \
  -graft-points \
